...
def class_seven_page(request):
    # Assuming you have a queryset for all students in class seven
    all_students = StudentDetail.objects.filter(standard='7')

    # Logic to find the highest and lowest GPA students
    highest_gpa_students = all_students.order_by('-gpa')[:1]
    lowest_gpa_students = all_students.order_by('gpa')[:1]

    # Count of male, female, and total students
    male_count = all_students.filter(gender='Male').count()
    female_count = all_students.filter(gender='Female').count()
    total_count = all_students.count()

    context = {
        'all_field': all_students,
        'male': male_count,
        'female': female_count,
        'total': total_count,
        'highest_gpa_students': highest_gpa_students,
        'lowest_gpa_students': lowest_gpa_students
    }

    return render(request, 'administrator/std.html', context)
